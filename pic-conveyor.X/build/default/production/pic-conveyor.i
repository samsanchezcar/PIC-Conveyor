...
EstadoColor_t rgb_color = COLOR_MAGENTA;
DutyPWM_t PWM_Duty = PWM_ZERO;


volatile unsigned char tecla = 0, input = 0;
volatile _Bool block = 0, ok = 0, ask = 0, rst = 0;


volatile unsigned char objective, missing, count, counter, last, curr;
volatile _Bool pressed = 0, inactive, emergency;
volatile _Bool medioSegundo;
...
unsigned char d = 255;
volatile unsigned char etimeout = 1;
volatile unsigned char ctimeout = 0;
                ...
                while(1) {
                    if(count != objective) {
                        do {
                            ...
                            _delay((unsigned long)((75)*(1000000/4000.0)));
                        } while(d < 5 || d > 8);
